  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl md:max-w-2xl flex flex-col h-[75vh]">
              ...
              <CommandGroup>
                {sortedAndFilteredFeats.map((featDef) => {
                  ...
                  return (
                    ...
                    >
                      <div className="font-medium text-sm text-foreground">{featDef.label} {featDef.isCustom && <span className="text-xs text-primary/70">(Custom)</span>}</div>
                      {featDef.description && (
                        <div
                      ...
                      {prereqMessages.length > 0 ? (
                        <p className="text-xs mt-0.5 whitespace-normal">
                         ...
                         <p className="text-xs mt-0.5 whitespace-normal text-muted-foreground/80"><strong className="text-foreground">Prerequisites:</strong> None</p>
                      )}
                    </CommandItem>
                  );
                })}
        ...
        <DialogFooter className="mt-4 pt-4"> {/* Removed border-t */}
