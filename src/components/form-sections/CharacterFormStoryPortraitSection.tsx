...
const CharacterFormStoryPortraitSectionComponent = ({
  ...
  const [localPersonalStory, setLocalPersonalStory] = useDebouncedFormField(
    storyAndAppearanceData.personalStory || '', React.useCallback((value) => onFieldChange('personalStory', value), [onFieldChange]), DEBOUNCE_DELAY
  );
  const [localHomeland, setLocalHomeland] = useDebouncedFormField(
    storyAndAppearanceData.homeland || '', React.useCallback((value) => onFieldChange('homeland', value), [onFieldChange]), DEBOUNCE_DELAY
  );
  const [localHeight, setLocalHeight] = useDebouncedFormField(
    storyAndAppearanceData.height || '', React.useCallback((value) => onFieldChange('height', value), [onFieldChange]), DEBOUNCE_DELAY
  );
  const [localWeight, setLocalWeight] = useDebouncedFormField(
    storyAndAppearanceData.weight || '', React.useCallback((value) => onFieldChange('weight', value), [onFieldChange]), DEBOUNCE_DELAY
  );
  const [localEyes, setLocalEyes] = useDebouncedFormField(
    storyAndAppearanceData.eyes || '', React.useCallback((value) => onFieldChange('eyes', value), [onFieldChange]), DEBOUNCE_DELAY
  );
  const [localHair, setLocalHair] = useDebouncedFormField(
    storyAndAppearanceData.hair || '', React.useCallback((value) => onFieldChange('hair', value), [onFieldChange]), DEBOUNCE_DELAY
  );
  const [localSkin, setLocalSkin] = useDebouncedFormField(
    storyAndAppearanceData.skin || '', React.useCallback((value) => onFieldChange('skin', value), [onFieldChange]), DEBOUNCE_DELAY
  );

  if (translationsLoading || !translations) {
    return (
      <LockablePanelWrapper
        title={translations?.UI_STRINGS.storyPortraitPanelTitle || "Personal Story & Appearance"}
        description={translations?.UI_STRINGS.storyPortraitPanelDescription || "Flesh out your character's background and physical details."}
        icon={UserSquare2}
        cardContentClassName="space-y-4 pt-2"
  ...
  return (
    <LockablePanelWrapper
      title={UI_STRINGS.storyPortraitPanelTitle || "Personal Story & Appearance"}
      description={UI_STRINGS.storyPortraitPanelDescription || "Flesh out your character's background and physical details."}
      icon={UserSquare2}
      cardContentClassName="space-y-4 pt-2"
      initialLockedState={false}
    >
          ...
          <div className="mt-4 pt-4 border-t border-border/40">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  <div className="space-y-1.5">
                      ...
                      <Input id="height" name="height" value={localHeight} onChange={(e) => setLocalHeight(e.target.value)} placeholder={UI_STRINGS.heightPlaceholder || "e.g., 5'10\""} disabled={panelIsLocked}/>
                  </div>
                  <div className="space-y-1.5">
                      <Label htmlFor="weight">{UI_STRINGS.weightLabel || "Weight"}</Label>
                      <Input id="weight" name="weight" value={localWeight} onChange={(e) => setLocalWeight(e.target.value)} placeholder={UI_STRINGS.weightPlaceholder || "e.g., 160 lbs"} disabled={panelIsLocked}/>
                  </div>
                   <div className="space-y-1.5">
                      <Label htmlFor="eyes">{UI_STRINGS.eyesLabel || "Eyes"}</Label>
                      <Input id="eyes" name="eyes" value={localEyes} onChange={(e) => setLocalEyes(e.target.value)} placeholder={UI_STRINGS.eyesPlaceholder || "e.g., Hazel"} disabled={panelIsLocked}/>
                  </div>
                  <div className="space-y-1.5">
                      <Label htmlFor="hair">{UI_STRINGS.hairLabel || "Hair"}</Label>
                      <Input id="hair" name="hair" value={localHair} onChange={(e) => setLocalHair(e.target.value)} placeholder={UI_STRINGS.hairPlaceholder || "e.g., Raven Black, Tousled"} disabled={panelIsLocked}/>
                  </div>
                  <div className="space-y-1.5">
                      <Label htmlFor="skin">{UI_STRINGS.skinLabel || "Skin"}</Label>
                      <Input id="skin" name="skin" value={localSkin} onChange={(e) => setLocalSkin(e.target.value)} placeholder={UI_STRINGS.skinPlaceholder || "e.g., Fair, Tanned"} disabled={panelIsLocked}/>
                  </div>
              </div>
          </div>
        </>
