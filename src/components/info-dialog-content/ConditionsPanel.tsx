...
const formatEffectSummary = (
  ...
  abilityLabels: readonly { id: string; label: string; abbr: string }[],
  isGloballyActive: boolean
): React.ReactNode[] => {
  ...
  effects.forEach((effect, index) => {
    ...
    switch (effect.type) {
      case "abilityScore":
        ...
        effectTargetDisplay = abilityLabels.find(ab => ab.id === asEffect.ability)?.abbr || asEffect.ability.toUpperCase();
        break;
      case "savingThrow":
      ...
      case "armorClass":
        ...
        if (typeof val === 'number') {
          ...
          effectValueDisplay = abilityLabels.find(al => al.id === String(val).toLowerCase())?.abbr || String(val);
        } else {
...
const ConditionsPanelComponent: React.FC<ConditionsPanelProps> = ({
  ...
  const uniqueConditionsMap = React.useMemo(() => {
    ...
    conditionsMap.forEach(entry => {
      ...
      const keyName = `condition${entry.conditionKey.charAt(0).toUpperCase() + entry.conditionKey.slice(1)}` as keyof typeof translations.UI_STRINGS;
      entry.displayText = translations.UI_STRINGS[keyName] || capitalizeFirstLetter(entry.conditionKey);

      entry.sources.forEach(source => {
         const definition = allFeatDefinitions.find(def => def.label === source.featName || def.id === source.featName);
         
         if (entry.conditionKey === "vsFavoredEnemy" && aggregatedFeatEffects?.favoredEnemyBonuses) {
            ...
            source.effectsSummary = formatEffectSummary(
                ...
                translations.ABILITY_LABELS as any,
